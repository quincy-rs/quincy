...
impl QuincyInstance {
    ...
    async fn start(name: String, config_path: PathBuf) -> Result<Self> {
        ...
        let child = Self::spawn_daemon_process(&daemon_binary, &config_path, &name)?;
        let ipc_client = Self::establish_ipc_connection(&name).await;

        let mut instance = Self::create_instance(name, child, ipc_client);
    ...
    ///
    /// # Errors
    /// Returns an error if the process cannot be spawned with elevated privileges
    fn spawn_daemon_process(daemon_binary: &Path, config_path: &Path, name: &str) -> Result<Child> {
        run_elevated(
    ...
    async fn stop(&mut self) -> Result<()> {
        ...
        self.wait_for_process_shutdown().await;
        self.reset_status();
    ...
    /// Waits for the daemon process to shutdown gracefully, then force kills if necessary.
    async fn wait_for_process_shutdown(&mut self) {
        if let Some(mut process) = self.process.take() {
            const MAX_WAIT_ITERATIONS: u32 = 30;
            const WAIT_INTERVAL: Duration = Duration::from_millis(100);

            if self
                .wait_for_graceful_exit(&mut process, MAX_WAIT_ITERATIONS, WAIT_INTERVAL)
                .await
            {
                return;
            }
    ...
    /// Waits for the process to exit gracefully.
    ///
    /// # Arguments
    /// * `process` - Process handle to monitor
    /// * `max_iterations` - Maximum number of wait cycles
    /// * `wait_interval` - Time to wait between checks
    ///
    /// # Returns
    /// `true` if process exited gracefully, `false` if timeout reached
    async fn wait_for_graceful_exit(
        ...
        for _ in 0..max_iterations {
            let wait_result = process.try_wait();

            match wait_result {
                Ok(Some(_)) => {
                    info!("Daemon process exited gracefully");
                    return true;
                }
                Err(e) => {
                    warn!("Error checking daemon process status: {}", e);
                    return false;
                }
                Ok(None) => {
    ...
    /// Force kills the daemon process.
    ///
    /// # Arguments
    /// * `process` - Process handle to kill
    fn force_kill_process(&self, process: &mut Child) {
        warn!("Daemon didn't exit gracefully within 3 seconds, force killing");
        let _ = process.kill();
        let _ = process.wait();
    }
...
impl QuincyGui {
    ...
    /// setting up the configuration directory, and loading existing configurations.
    ///
    /// # Returns
    ...
    fn build_monitoring_section(
        ...
        if let Some(instance) = self.instances.get(&selected_config.quincy_config.name) {
            ...
            column![text("Loading status...")].into()
        }
    }
    ...
    fn build_instance_status_display(&self, status: &ClientStatus) -> Element<Message> {
        let mut col = column![
            ...
            text(format!("{:?}", status.status)).size(14),
        ];

        if let Some(ref metrics) = status.metrics {
            col = col.push(self.build_metrics_display(metrics));
        }

        col.spacing(5).into()
    }

    /// Builds the metrics display section.
    ///
    /// # Arguments
    ...
    fn build_metrics_display(&self, metrics: &quincy::ipc::ConnectionMetrics) -> Element<Message> {
        column![
            ...
            row![
                text(format!("↑ {}", format_bytes(metrics.bytes_sent))),
                text(format!("↓ {}", format_bytes(metrics.bytes_received)))
            ]
            .spacing(20),
            text(format!(
            ...
            .size(12),
            text(format!("Connected for: {:?}", metrics.connection_duration)).size(12),
        ]
        .spacing(5)
        .into()
    }
    ...
    fn build_action_buttons(&self, has_client: bool) -> Element<Message> {
        ...
        let delete_button = column![button(text("Delete"))
...
/// ```
/// // Unix
/// let expanded = expand_path(Path::new("~/.config/app"));
/// // Returns: "/home/user/.config/app"
///
/// // Windows
/// let expanded = expand_path(Path::new("%AppData%/app"));
/// // Returns: "C:/Users/user/AppData/Roaming/app"
/// ```
...
///
/// This function initializes logging, configures the window settings,
/// and starts the Iced GUI framework with the QuincyGui application.
///
/// # Returns
